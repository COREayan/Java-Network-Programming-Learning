    ...
    public static void main(String[] args) {
        ...
        while (!isValid) {
            try {
                ...
                if (toPort >= 0 && toPort <= 65536) { // verify that port is in the valid range
                    if (toPort >= fromPort) { // verify if toPort is greater than fromPort
                        isValid = true; // set the boolean valud to true and exit the while loop 
                    }
                } else {
                    System.out.println("Invalid port! Port range is: 0-65536"); // inform user about the valid port range
                }
        ...
        while (port >= fromPort && port <= toPort) { // use the given port range
            try {
                ...
                System.out.println("Port " + port + " is not openend!"); // print what port is opened
